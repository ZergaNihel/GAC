...
<input type="hidden" id="curr_sem" value="{{$semestre->idSem}}">
       ...
       @if(Auth::user()->role == '1' or Auth::user()->role =='2')                           <h4><a class="cards-hd-dn" href="#">{{Auth::user()->name}}</a></h4>
        @if(Auth::user()->role == '1' )
                                <h5>Admin</h5>
        @endif
         @if(Auth::user()->role == '2' )
                                <h5>équipe d'anonyma</h5>
        @endif
                                <p class="ctn-cards">Lorem ipsum dolor sit amet, this is a consectetur adipisicing elit</p>
                                 <a class="follow-cards" href="{{url('membre/'.Auth::user()->id.'/details')}}">Profil</a>
                                @endif
